...
NA.values<-colSums(is.na(train),na.rm = TRUE)
NA.values
```
#2.
...
round(summary(mod.fit.logit.in)$coefficients,4)
```

```{r}
mod.fit.probit.in <- glm( type ~ age+bmi+age:bmi, family=binomial(link=probit), data=train)
round(summary(mod.fit.probit.in)$coefficients,4)
...
round(summary(mod.fit.cloglog.in)$coefficients,4)
```
...
probab.mod
```
Si la persona de 35 anos presenta un bmi de 32 tiene una probabilidad de casi 35% de padecer diabetes, mientras que una persona de la misma edad pero con un bmi de 22 tiene una probabilidad de casi el 16% de presentar diabetes.
...
No, debido a que los modelos logit, probit y log log complementario  con interacci??n se rechazan ya que el p- value es mayor a nuestro valor de significancia (0.05)  por lo que la variable age:bmi no tiene relaci??n con el modelo.
